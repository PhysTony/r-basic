...
```{r}
y_1 <- function(x){x^2 - 3*x + 30}
curve(y_1, xlim = c(-15,15),
     xlab = expression(x), ylab = expression(x^2 - 3*x + 30)) # Es lo mismo si se usa plot()
```
...
**R:** En este caso, habrá diferencia ya que se graficarán puntos, en aprticular, 31.
```{r}
y_1 <- function(x){x^2 - 3*x + 30}
plot(y_1(c(-15:15)), 
